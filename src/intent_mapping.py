"""
Маппинг ID -> имя интента для датасета CLINC150 (clinc_oos, plus).

Используется во всех экспериментах для перевода целочисленных меток модели
в человекочитаемые имена интентов.

Порядок совпадает с порядком в HuggingFace clinc_oos[plus].features["intent"].names.
"""
...
# Обратный маппинг: имя интента -> ID
INTENT_TO_ID = {name: idx for idx, name in ID_TO_INTENT.items()}

assert len(ID_TO_INTENT) == 151, "Должно быть 151 интент (150 + oos)"
assert len(INTENT_TO_ID) == 151, "В именах есть дубликаты"
